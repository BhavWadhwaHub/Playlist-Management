...
private:
	...
	{
	private:
		int currentSize;  	//The number of elements currently in the stack
		int capacity;		//How many elements it can hold
		string* arr;		// Pointer to a dynamic array holding the stack's elements

		// Doubles the capacity of the stack when it reaches its current limit
		// POST: The capacity of the stack is doubled, and the elements are transferred to the new array
		void increaseCapacity();
	public:
		// Default constructor for the stack
		// POST: Initializes the stack with a capacity of 10 and sets currentSize to 0
		Stack();

		// Destructor for the stack
    	// POST: Frees the dynamically allocated memory for the stack's array
		~Stack();

		 // Pushes a new string onto the top of the stack
    	// PARAM: The string to be pushed onto the stack
    	// POST: The string is added to the top of the stack; capacity is increased if necessary
		void push(const string&);

		// Pops the top string off the stack and returns it
    	// POST: Removes and returns the top string of the stack. Throws an error if the stack is empty.
		string pop();
